  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "fixed z-50 flex flex-col gap-4 border border-border bg-background p-6 shadow-2xl transition",
          side === "right" && "inset-y-0 right-0 h-full w-full max-w-sm border-l",
          side === "left" && "inset-y-0 left-0 h-full w-full max-w-sm border-r",
          side === "top" && "inset-x-0 top-0 h-auto max-h-[80vh] rounded-b-2xl border-b",
          side === "bottom" && "inset-x-0 bottom-0 h-auto max-h-[80vh] rounded-t-2xl border-t",
          className
        )}
        {...props}
      >
